...
def write_starter_file(output_path, nodes, elements, elset_elements):
    ...
    if 'Solid_part-1' in elset_elements:
        parts[1] = {'name': 'Punch', 'role': 'PUNCH', 'elements': elset_elements['Solid_part-1'], 'mat_id': 1, 'prop_id': 1}
    material_elements = []
    if 'Solid_part-5' in elset_elements: material_elements.extend(elset_elements['Solid_part-5'])
    if 'Solid_part-6' in elset_elements: material_elements.extend(elset_elements['Solid_part-6'])
    ...
    if 'Solid_part-4' in elset_elements:
        parts[3] = {'name': 'Die', 'role': 'DIE', 'elements': elset_elements['Solid_part-4'], 'mat_id': 1, 'prop_id': 1}
    ...
    with open(output_path, 'w') as f:
        ...
        f.write("                  kg                   m                   s\n")
        f.write("                  kg                   m                   s\n")
        f.write("/TITLE\nPunch Die Shearing - V7 with Sol2SPH (Solid to SPH Conversion)\n")
        f.write("/NODE\n")
        ...
        # Solid Property for Material (Isolid=1 for Sol2SPH compatibility)
        # Sol2SPH: Add Ndir and SPH_part on additional line to convert failed elements to SPH
        f.write("/PROP/SOLID/2\nMaterial_Solid_Sol2SPH\n")
        f.write("#   Isolid    Ismstr                               Dn                Qa                Hm\n")
        f.write("         1         4             0.00000             0.00000             1.00000\n")
        f.write("                   0                   0                   0                   0                   0\n")
        f.write("#     Ndir   SPH_part\n")
        f.write("         3       200\n")
        
        # SPH Property for converted particles (/PROP/TYPE34 = SPH)
        f.write("/PROP/TYPE34/200\nMaterial_SPH_Property\n")
        f.write("#                h0              Qa               Qb\n")
        f.write("         0.00050           1.000           1.000\n")
        ...
        # SPH Part for Sol2SPH conversion target
        f.write("/PART/200\nMaterial_SPH\n#    Prop_ID     Mat_ID\n       200         2\n")
        ...
        # Sol2SPH is now configured inside /PROP/SOLID/2 with Ndir and SPH_part
        
        # AMS (Advanced Mass Scaling) for fine blanking
        # Note: /AMS is recognized by Starter, but /DT/AMS not supported in OSS Engine
        # Using aggressive Mass Scaling (Tmin=1E-6) for ~5 hour run time with Sol2SPH
        f.write("/AMS\n")
        f.write("/DT/NODA/CST\n#               Tmin            Tscale\n         1.0E-06             0.90000\n/END\n")
...
def write_engine_file(output_path):
    ...
    with open(output_path, 'w') as f:
        ...
        # Note: /DT/AMS not supported in OpenRadioss open-source version
        # Using enhanced Mass Scaling (Tmin=5E-7) in Starter instead
